...
Trading simulator with SHA-256 + salted auth, multithreaded price feeds, and custom DSA (Priority Queue, Stack, BST) built from scratch. Huffman-compressed export pipeline. Containerised with Docker.

`C++` `SQL Server` `Docker` `DSA`
...
Flask REST API with JWT auth, CRUD for pet listings and adoption requests, and an Amazon RDS (MySQL) backend consumed by a vanilla JS frontend.

`Python` `Flask` `REST API` `AWS RDS` `JavaScript`
